	...
	if ($("#confirminfoform").form("is valid")) {
		...
		for (artist in cartEmailItems) {
			...
			for (id in cartEmailItems[artist]) {
				itemsHTML += `<tr>
					<td style="-webkit-transition: background .1s ease,color .1s ease;transition: background .1s ease,color .1s ease;padding: .78571429em .78571429em;text-align: inherit;border-top: 1px solid rgba(34,36,38,.1);"><img src="https://atkhrfnsco.cloudimg.io/v7/snsartsfestival.in/img/snsnartshaat/${cartEmailItems[artist][id].img}?w=256&amp;height=256" class="ui image tiny" style="border-style: none;position: relative;display: block;vertical-align: middle;max-width: none;background-color: transparent;width: 80px;height: auto;font-size: .85714286rem;"></td>
...
$('.cartbutton').click(function () {
	...
	html = ''
	for (id in cartItems) {
		html += `<tr><td><img src="https://atkhrfnsco.cloudimg.io/v7/snsartsfestival.in/img/snsnartshaat/${cartItems[id].img}?w=256&h=256" class="ui image tiny"></td><td><h4>${cartItems[id].name}</h4></td></td><td><h4>₹ ${cartItems[id].price}</h4></td></tr>`
	}
